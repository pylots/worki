    ...
    try:
        ...
        if wiki_page.ttl is not None:
            ...
            if ttl < datetime.now(pytz.utc):
                expired = True
        return render(request, 'wiki_page/show.html', 
...
def edit(request, slug):
    ...
    try:
        ...
        if post:
            ...
            form = WikiPageForm()
    return render(request, 'wiki_page/edit.html', {'form': form, 'url': request.path})
...
def pages_by_tag(request, tag):
    ...
    return render(request, 'wiki_page/index.html', {'object_list': page_list, 'head': tag, 'url': request.path})
...
def search(request):
    ...
    return render(request, 'wiki_page/index.html', {'object_list': object_list, 'head': 'Search', 'url': request.path})
    
def pages_by_user(request, user):
    user_list = WikiPage.objects.filter(author__username=user)
    return render(request, 'wiki_page/index.html', {'object_list': user_list, 'head': user, 'url': request.path})
